---
description: Canonical criteria, scope, and tone for reviewing pull requests across the organization.
---

# Pull Request Review

How to review a PR, whether the reviewer is a human or an AI agent (e.g., the `claude-review.yml` workflow). The goal is feedback that improves the change without slowing it down.
...
- **Tests:** Are new behaviors covered? Do tests assert outcomes rather than implementation details? Are negative and edge cases covered?
...
- **Style & typing:** Spot-check only; trust `ruff` and `pyright` for the rest. Do not re-flag what tooling already enforces.
- **Documentation:** READMEs, in-code comments, and `CLAUDE.md` updated in the same change when behavior, commands, or environment variables shift.
...
- **Severity tags optional:** `blocker`, `concern`, `nit`. Default is `concern`. Reserve `blocker` for correctness or security.
- **Praise sparingly.** A short note on a non-obvious good decision is useful; reflexive praise on every PR is noise.

## When to Approve

Approve when the change is correct, scoped, and tested — even if minor nits remain. Do not block on items the author can address in a follow-up unless they materially affect correctness or security.

## Delivery

When reviewing via the `claude-review.yml` workflow, deliver feedback as:
